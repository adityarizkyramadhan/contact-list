...
type (
	...
	ContactRepository interface {
		...
		FindAll(ctx context.Context, userID uuid.UUID, query request.ContactQuery) ([]Contact, error)
	...
	ContactUsecase interface {
		...
		FindAll(ctx context.Context, userID string, query request.ContactQuery) ([]Contact, error)
